...
uint8_t Continus_Count = 0;
IMU_Measure_StateMachine_Type Current_IMU_Measure_State;
IMU_AcData_Type Current_AcData;
IMU_AcData_Type Prv_AcData[IMU_DATA_BATCH];
IMU_AcData_Type AcData_Array[ACDATA_ARRAY_NUM];

IMU_GyData_Type Current_GyData;
IMU_GyData_Type Prv_GyData[IMU_DATA_BATCH];
IMU_GyData_Type GyData_Array[ACDATA_ARRAY_NUM];

bool IMU_Start_Point_Check(void){
  if(abs(Prv_AcData[IMU_DATA_BATCH - 1].AcX - Prv_AcData[IMU_DATA_BATCH - 2].AcX)>=1 
  && abs(Prv_AcData[IMU_DATA_BATCH - 1].AcY - Prv_AcData[IMU_DATA_BATCH - 2].AcY)>=1
  && abs(Prv_AcData[IMU_DATA_BATCH - 1].AcZ - Prv_AcData[IMU_DATA_BATCH - 2].AcZ)>=1
   ){
    return true;
  }
...
void IMU_Read_Average_Ac(uint16_t READ_NUM, IMU_AcData_Type * OutAcData){
  ...
  for(uint16_t count = 0; count <=(READ_NUM - 1); count++)
  {
    ...
    SumAcData.AcX += (TempAcData.AcX/400 + 128);  
    SumAcData.AcY += (TempAcData.AcY/400 + 128);
    SumAcData.AcZ += (TempAcData.AcZ/400 + 128);
  }
...
void IMU_Read_Average_Gy(uint16_t READ_NUM, IMU_GyData_Type * OutGyData){
  ...
  for(uint16_t count = 0; count <=(READ_NUM - 1); count++)
  {
    ...
    SumGyData.GyX += (TempGyData.GyX/400 + 128);  
    SumGyData.GyY += (TempGyData.GyY/400 + 128);
    SumGyData.GyZ += (TempGyData.GyZ/400 + 128);
  }
...
void IMU_Init(void){
  ...
  Current_IMU_Measure_State = MEASURE_NORMAL;
  Prv_AcData[0].AcX = 0;
  Prv_AcData[0].AcY = 0;
  Prv_AcData[0].AcZ = 0;
